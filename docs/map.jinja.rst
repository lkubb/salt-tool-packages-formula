...
We will see a quick setup to configure the ``tool_pkgs`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_pkgs`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_pkgs`` formulas under ``/srv/salt/tool_pkgs/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_pkgs/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_pkgs/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_pkgs/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_pkgs/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_pkgs`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_pkgs`` formulas under ``/srv/salt/tool_pkgs/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_pkgs/parameters/roles
...
We create a configuration for the role ``tool_pkgs/server`` in ``/srv/salt/tool_pkgs/parameters/roles/tool_pkgs/server.yaml``:
...
We create another configuration for the role ``tool_pkgs/client`` in ``/srv/salt/tool_pkgs/parameters/roles/tool_pkgs/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_pkgs/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_pkgs/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_pkgs/parameters/dns:domain/  '/srv/salt/tool_pkgs/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-pkgs-as-json:
      test.nop:
        - name: {{ pkgs | json }}
...
.. code-block:: sls
    ...
    pkgs-config-file-file-managed:
      file.managed:
        - name: {{ pkgs.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='pkgs-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ pkgs.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``pkgs`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ pkgs['winner'] }}
